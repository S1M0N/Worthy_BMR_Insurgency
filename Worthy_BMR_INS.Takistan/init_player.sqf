...
if (DebugEnabled > 0) then {
	...
	[] spawn {
		...
		if (local player) then {
			...
			//player addAction[("<t color=""#00ffe9"">") + ("Load VAS profile") + "</t>","=BTC=_revive\=BTC=_addAction.sqf",[[],BTC_main_load_VAS], 1, true, true, "", "true"];
...
[] spawn {
	...
	// VAS
	//INS_Wep_box addAction["<t color='#ff1111'>Virtual Ammobox</t>", "VAS\open.sqf"];
