...
INS_intro = {
	...
	//_camPos = [(getPosatl INS_flag select 0) - 1, (getPosatl INS_flag select 1) + 1, (getTerrainHeightASL (position INS_flag)) + 30];
	_camPos = [(getPosatl INS_flag select 0) - 1, (getPosatl INS_flag select 1) + 1, (getpos INS_flag select 2) + 30];
...
BTC_main_load_VAS = {
	...
	for "_i" from 0 to 15 do
	{
		if (!isNil {profileNameSpace getVariable format ["vas_gear_new_%1",_i]}) then
		{
			_i spawn
			{
				_id = INS_Wep_box addAction [("<t color=""#00ffe9"">") + ("Load " + format ["%1",(profileNameSpace getVariable format ["vas_gear_new_%1",_this]) select 0]) + "</t>","=BTC=_revive\=BTC=_addAction.sqf",[[format ["vas_gear_new_%1",_this]],BTC_load_VAS], 8, true, true, "", "true"];
				sleep 15;
...
BTC_load_VAS = {
	// Quick VAS profile loadout
	_id = _this select 0;
	_loadout = profileNameSpace getVariable _id;//hint format ["%1 - %2",_id,_loadout];
...
JIG_p_actions_resp = {
	...
	if (_playertype in INS_W_PlayerJTAC) then {player execVM "supplyDrop\addAction.sqf";};
	// Medic
