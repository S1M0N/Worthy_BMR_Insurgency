...
mhq_actions_fnc = {
	...
	switch (true) do {
		case (_var isEqualTo "MHQ_1") : {_veh addAction [("<t color=""#F56618"">") + ("Load VAS profile") + "</t>",{[] spawn BTC_MHQ1_load_VAS;},nil, 1, true, true, "", "true"]; _veh addAction["<t color='#ff1111'>Virtual Ammobox</t>", "VAS\open.sqf"];};
		case (_var isEqualTo "MHQ_2") : {_veh addAction [("<t color=""#F56618"">") + ("Load VAS profile") + "</t>",{[] spawn BTC_MHQ2_load_VAS;},nil, 1, true, true, "", "true"]; _veh addAction["<t color='#ff1111'>Virtual Ammobox</t>", "VAS\open.sqf"];};
		case (_var isEqualTo "MHQ_3") : {_veh addAction [("<t color=""#F56618"">") + ("Load VAS profile") + "</t>",{[] spawn BTC_MHQ3_load_VAS;},nil, 1, true, true, "", "true"]; _veh addAction["<t color='#ff1111'>Virtual Ammobox</t>", "VAS\open.sqf"];};
...
BTC_MHQ1_load_VAS = {
	...
	for "_i" from 0 to 15 do {
		if (!isNil {profileNameSpace getVariable format ["vas_gear_new_%1",_i]}) then
		{
			_i spawn
			{
				_id = MHQ_1 addAction [("<t color=""#00ffe9"">") + ("Load " + format ["%1",(profileNameSpace getVariable format ["vas_gear_new_%1",_this]) select 0]) + "</t>","[format ['vas_gear_new_%1',_this]] spawn BTC_load_VAS;",nil, 8, true, true, "", "true"];
				sleep 15;
...
BTC_MHQ2_load_VAS = {
	...
	for "_i" from 0 to 15 do
	{
		if (!isNil {profileNameSpace getVariable format ["vas_gear_new_%1",_i]}) then
		{
			_i spawn
			{
				_id = MHQ_2 addAction [("<t color=""#00ffe9"">") + ("Load " + format ["%1",(profileNameSpace getVariable format ["vas_gear_new_%1",_this]) select 0]) + "</t>","[format ['vas_gear_new_%1',_this]] spawn BTC_load_VAS;",nil, 8, true, true, "", "true"];
				sleep 15;
...
BTC_MHQ3_load_VAS = {
	...
	for "_i" from 0 to 15 do {
		if (!isNil {profileNameSpace getVariable format ["vas_gear_new_%1",_i]}) then
		{
			_i spawn
			{
				_id = MHQ_3 addAction [("<t color=""#00ffe9"">") + ("Load " + format ["%1",(profileNameSpace getVariable format ["vas_gear_new_%1",_this]) select 0]) + "</t>","[format ['vas_gear_new_%1',_this]] spawn BTC_load_VAS;",nil, 8, true, true, "", "true"];
				sleep 15;
