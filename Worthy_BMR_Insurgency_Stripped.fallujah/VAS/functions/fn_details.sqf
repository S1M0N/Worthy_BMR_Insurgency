...
//_items = [_data] call VAS_fnc_accList;//Tonic's original
_items = if (isNil "asdg_jointrails_fnc_getCompatibleAttachments") then {[_data] call VAS_fnc_accList} else {[_data] call asdg_jointrails_fnc_getCompatibleAttachments};// joint rails fix by Robalo
