...
#define westPlayerStrings ["tester", "theOtherTester"]
...
if (isNil("KRON_UPS_INIT")) then {
	...
	if (DEBUG) then {KRON_UPS_Debug=1};
...
_getAreaInfo = {
	_obj = ((nearestPlayers(getPosATL _this select 0,20000,true,"array")+startLocation) select 0);
...
while {_loop} do {
	...
	if (count _members==0) then {
		_exit=true;
		if (KRON_UPS_Debug>0) then { server globalChat format["UPS group %1 all dead or surrendered", _grpidx]; };
		sleep 5;
		deleteGroup (group _npc);
		sleep 300+(random aiVehicleRespawnTime);
		(call compile _grpidx) call SR_fnc_spawnAIVehicle;
	} else {
		// did the leader die?
		if (!alive _npc) then {
